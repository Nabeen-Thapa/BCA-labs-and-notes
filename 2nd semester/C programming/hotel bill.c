...
int chhoyala=120,chickenpizza=500,chickentanduri=190,sukuti=200,chickenmomo=250,buffmomo=200,masu=500;
int num,i,total,finaltotal,name;
float discount;
void price();
void finalbill();
int totalamount();
...
for(i=0;i<num;i++)
{
printf("\n\nenter name of your item %d: ",1+i);
scanf("%s",&b[i].item);	
printf("enter how %s you want: ",b[i].item);
scanf("%d",&b[i].num);
price();
}
printf("\n\n");
/*printf("Tha bill of Mr./Mrs %s",name);
printf("\n\n\t\t\t\t\tNew NT hotel  \n");
...
if(strcmp(b[i].item,"blacktea")==0)
	...
	else if(strcmp(b[i].item,"masu")==0)
	printf("\n\tMasu\t\t\t%d\t\t  %d\t\t\t%d",masu,b[i].num,masu*b[i].num);
}
}
...
for(i=0;i<=num;i++)
{
	...
	total=total+masu*b[i].num;
	  ...
	  printf("\tFinal total\t\t\t\t\t\t\t%d",finaltotal);       
	...
	printf("your amount for Masu is %d",masu*b[i].num);
	else
